...
class ParticipantData(ParticipantDataBase):
    ...
    def test_trials(self):
        ...
        for t in self.trials:
            if t['trial_type'] == "GridNavigation" and t['data']['trialparams']['roundtype'] in ['test']:
                tt.append(GridNavigationTrialData(t))
        assert [(ta.trial_index + 1)== tb.trial_index for ta, tb in zip(tt, tt[1:])]
        return tt
    ...
    def plot_trials(self, nrows=2, figsize_mult=5):
        ...
        suptitle_kw = [
            'sessionId', 'bonusDollars', 'training_condition', 
            'generalComments', 'greenSquareThink',
            'winPointsThink', 'brokenBlocksThink',
            'gender', "age", 'mean_test_steps', 'total_time'
        ]
...
class ExperimentDataLoader:
    ...
    def test_trials(self, show_progress=False):
        p_test = []
        iterator = list(self.completed_participant_data())
        if show_progress:
