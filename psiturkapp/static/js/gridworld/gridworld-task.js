...
export class GridWorldTask {
    ...
    init({
        ...
        let tile_params = fromPairs(Object.keys(this.mdp.location_features).map((s) => {
            let f = this.mdp.location_features[s]
            return [s, {fill: feature_colors[f]}]
        }));
        this.painter.draw_tiles(tile_params);

        let init_state = this.mdp.initial_state();
    ...
    _process_action({action}) {
        ...
        return {
            state,
            state_type: this.mdp.location_features[state],
            action,
            nextstate,
            nextstate_type: this.mdp.location_features[nextstate],
            reward,
