...
async function runExperiment(display_element) {
    ...
    /* Set up participant on server */
    let condition = searchParams.get('condition');

    if (window.location.pathname !== "/testexperiment") {
        let codeversion = config['params']['EXPERIMENT_CODE_VERSION'] || "NO_EXPERIMENT_CODE_VERSION";
        let codeversion_res =
            await fetch(`/set_codeversion?codeversion=${codeversion}&uniqueId=${uniqueId}`).
            then(r => r.json());
        if (codeversion_res['msg'] !== 'success') {
            throw new Error("Code version not set")
        }
        if (condition === null) {
            let n_conditions = config['timelines'].length;
            let get_condition_result = await fetch(
                '/get_condition?' +
                `n_conditions=${n_conditions}` +
                `&cutoff_time_min=${config['params']['cutoff_time_min'] || 30}` +
                `&mode=${mode}` +
                `&uniqueId=${uniqueId}` +
                `&experiment_code_version=${codeversion}`
            ).then(r => r.json())
            condition = get_condition_result['condition'];
            console.log(`Condition set to ${condition}`)
            console.log("Condition counts:")
            console.log(get_condition_result['condition_counts'])
        }
    }
    else {
        console.log("Setting condition only on client.")
